...
# Point Flask to the correct template & static directories (they were extracted inside timestamped folders)
templates_dir = os.path.join(basedir, 'templates-20250630T060529Z-1-001', 'templates')
static_dir    = os.path.join(basedir, 'static-20250630T060518Z-1-001', 'static')

# Provide these explicit paths so render_template can find the HTML files
...
def view_traders():
    ...
    return render_template('view_traders.html', traders=traders)   
...
def delete_trader(trader_id): 
    ...
    return redirect(url_for('view_traders'))
...
def view_portfolios():
    ...
    return render_template('view_portfolios.html', 
                          portfolios=portfolios, 
                          total_portfolio_value=total_portfolio_value)


@app.route('/buy_and_sell')
def buy_and_sell():
    if 'email' not in session or session.get('role') != 'trader':
        flash("Access denied. Traders only.", "danger")
        return redirect(url_for('login'))
    ...
    user = User.query.filter_by(email=session['email']).first()
    stocks = Stock.query.all()
    return render_template('buy_and_sell.html', user=user, stocks=stocks)

@app.route('/buy_and_sell/buy_stock/<int:stock_id>', methods=['GET', 'POST'])
...
def my_portfolio():
    ...
    return render_template('my_portfolio.html', user=user, portfolio=portfolio, total_value=total_value, transactions=transactions)
